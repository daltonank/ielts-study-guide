...
# T5-B appends THREE guarded post-migration stages (definitionUa remediation, issue #4) on top of
# the SB-0773 final: the adjacent/punctuation-separated stage (bb173f36), the connector-separated
# stage (7520f722), then the PR #7 follow-up stage (1c184e84). Earlier historical stages are NOT
# repinned; the chain simply grows by three stages ending at the new pinned final blob.
T5B_FINAL_BLOB = "bb173f3614dbf35558d96a17b7f692d28f82f303"  # adjacent stage output (intermediate)
T5B_CONNECTOR_FINAL_BLOB = "7520f7220a64df20240523cf12fcd08a70734bc0"  # connector stage output (intermediate)
# The PR #7 blocking-review remediation appends a FOURTH guarded post-migration stage
# (punctuation-aware connector repeats + reviewer/blind-sample full-rubric re-adjudication) on top
# of the connector stage. Earlier stages are NOT repinned; the chain grows by one stage ending at
# the new pinned final blob.
T5B_FOLLOWUP_FINAL_BLOB = "1c184e84e5c63e3a9f8e386af13787664a23bd66"  # follow-up stage output (final)
...
def main() -> int:
    ...
    assert t5bc["expected_output_blob_sha1"] == T5B_CONNECTOR_FINAL_BLOB, "T5-B connector step does not target its pinned blob"
    t5bf = payload_meta("t5b_followup_corrections.json")
    assert t5bf["expected_input_blob_sha1"] == T5B_CONNECTOR_FINAL_BLOB, "T5-B connector->follow-up chain broken"
    assert t5bf["expected_output_blob_sha1"] == T5B_FOLLOWUP_FINAL_BLOB, "T5-B follow-up step does not target the new final blob"

    # Preserve the checkout's EXACT bytes (which may be CRLF under core.autocrlf=true) so we can
    # restore them verbatim afterwards, while validating the blob against the canonical LF content.
    ...
    try:
        ...
        # 5. T5-B follow-up guarded stage reproduces the new final blob and lands the
        # punctuation-aware connector + full-rubric re-adjudication corrections. SB-0773 untouched.
        run("scripts/qa/apply_t5b_followup_fixes.py")
        assert disk_blob() == T5B_FOLLOWUP_FINAL_BLOB, (
            f"T5-B follow-up final blob {disk_blob()} != reviewed {T5B_FOLLOWUP_FINAL_BLOB}")
        t5bf_payload = json.loads(
            (ROOT / "scripts" / "qa" / "t5b_followup_corrections.json").read_text(encoding="utf-8"))
        after = {e["id"]: e for e in json.loads(
            re.search(r"window\.VOCABULARY=(\[.*\]);", VOCAB.read_text(encoding="utf-8"),
                      re.DOTALL).group(1))}
        for cid, corr in t5bf_payload["corrections"].items():
            assert after[cid]["definitionUa"] == corr["definitionUa"], (
                f"T5-B follow-up correction for {cid} did not land")
        assert after["SB-0773"]["definitionUa"] == SB0773_DEFINITION_UA, "T5-B follow-up disturbed SB-0773"
    finally:
        shutil.copy2(backup, VOCAB)

    # Restore must be byte-exact (never rewrite the working copy — e.g. LF->CRLF on a Windows
    # checkout). Validate the pinned blob against the CANONICAL LF-normalized content so a
    # core.autocrlf=true (CRLF) checkout passes while genuinely mixed/corrupt endings still fail:
    # normalizing only "\r\n"->"\n" leaves a lone "\r" (corrupt) in place, changing the blob.
    restored = VOCAB.read_bytes()
    assert restored == original_bytes, "checkout bytes were not preserved exactly after restore"
    assert blob(restored.replace(b"\r\n", b"\n")) == T5B_FOLLOWUP_FINAL_BLOB, (
        "restored content does not match the canonical LF final blob")
    print("G4-A SB-0773 SOURCE-CHAIN PASS: workbook -> base -> P0 -> T2 -> T3 -> T4 -> "
          ...
          f"{T5B_CONNECTOR_FINAL_BLOB} -> T5-B follow-up {T5B_FOLLOWUP_FINAL_BLOB} reproduced; "
          "SB-0773 provenance + stable id + Ukrainian fields retained; 35 adjacent + 64 connector "
          "+ 23 follow-up T5-B corrections landed")
    return 0
