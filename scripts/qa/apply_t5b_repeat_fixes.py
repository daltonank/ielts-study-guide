#!/usr/bin/env python3
"""Apply the reviewed G4-A T5-B definitionUa repeat-remediation batch (issue #4).

This is the post-migration guarded stage that follows the SB-0773 structural stamp
(final blob 9282d201...) in the deterministic correction chain. It removes confirmed
adjacent / punctuation-separated identical-word duplications in ``definitionUa`` that
the failed T5-B promotion packet surfaced but did not fix, and that the register never
captured. It edits ``definitionUa`` only (plus the reviewed ``translationQa`` stamp).

The workbook is NOT edited: ``definitionUa`` flows from Study Bank column D through
``scripts/migrate_vocabulary.py`` into the pinned migration base blob ``4ed00c96``,
and every Ukrainian correction in this project (P0/T2/T3/T4/SB-0773) is layered as a
post-migration guarded patch rather than at source, precisely so the frozen base blob
and its downstream pins are never repinned. This stage keeps that invariant.

Guards (fail-closed, deterministic, non-repeatable):
  * input git-blob must equal the pinned SB-0773 final blob (this stage's input);
...
PAYLOAD_PATH = ROOT / "scripts" / "qa" / "t5b_repeat_corrections.json"
...
def main() -> int:
    ...
    for item_id in sorted(corrections):  # deterministic ordering
        ...
        # P1 batch already reviewed; otherwise record a fresh T5-B stamp.
        existing = entry.get("translationQa", "")
        if existing.startswith("Reviewed — G4-A P1"):
            entry["translationQa"] = existing + f"; T5-B repeat fix ({meta['review_date']})"
        else:
            entry["translationQa"] = (
                f"Reviewed — G4-A T5-B repeat remediation applied ({meta['review_date']})"
            )
