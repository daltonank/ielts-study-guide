#!/usr/bin/env python3
"""
Apply the G4-A Ukrainian linguistic QA "P1 batch T2" corrections to
web/vocabulary.js.
...
corrections) was applied by scripts/qa/apply_p0_fixes.py. This script applies
the first batch (T2) of the remaining unresolved-P1 backlog: 104 corrections
(sorted by (category, id) — circular-definition 26 / grammar 60 / other 18),
including a reviewer override for SB-1629 (transport) so the entry carries both
its noun and verb senses. All 104 corrections are AI linguistic QA drafts
accepted for this batch; provenance is stamped P1.

This mirrors scripts/qa/apply_p0_fixes.py precisely:
  ...
  3. ONLY ua/definitionUa/pos/register are mutated (plus the translationQa
     provenance stamp on each changed entry). Nothing else is touched.
...
Compute-only bootstrap:
    python3 scripts/qa/apply_p1_t2_fixes.py --compute-only
...
Usage (from a clean checkout, repository root):
    python3 scripts/qa/apply_p1_t2_fixes.py
...
CORR_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'p1_t2_corrections.json')
...
def main():
    ...
    assert len(applied) == 104, f"expected 104 corrections applied, got {len(applied)}"
