...
> **Status history:** recorded as `Approved` when first written on 2026-09-06, by the same automated
> pass that produced the findings it rests on. Downgraded to `Proposed` the same day: nothing had
...
previously-unflagged entries found a further 27 issues (27/70 = 38.6% of that sample,
0 of them P0).

Both figures are stated as observed rates only. The spot-check sampled exclusively from
entries the first pass judged clean, so it measures a different population and cannot
corroborate the 37.8% bank-wide figure; if anything it implies the true rate is higher.
No bank-wide estimate is asserted, here or in the register — that needs a documented
estimator and sampling basis this audit did not build. What the spot-check does
establish is narrower and sufficient for this decision: the first pass is not
...
### Implications
- `G4-A` is `CHANGES REQUESTED`, scoped to `web/vocabulary.js` **and** the three Reading
  family labels generated by `scripts/build_reading_curriculum.py` into
  `web/reading_data.js` (findings `G4A-R-001`–`G4A-R-003`, case agreement). Writing Task 1
  Ukrainian content is the only area needing no changes from this pass. An earlier version
  of this entry said the scope was `web/vocabulary.js` only, while the register it cites
  recorded the Reading findings — that contradiction is corrected here;
- this is an **AI linguistic QA pass** and is labelled as such wherever it is cited. It does
  not constitute, replace or satisfy a native-speaker editorial review; that gate remains
  separate and unmet, and no `G4-A PASS` may be read as having met it;
...
**Date:** 2026-09-10 · **Status:** Proposed (Claude Code executor; requires reviewer + Dalton sign-off)
