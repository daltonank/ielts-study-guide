...
**T5-B connector-separated remediation (2026-09-10, branch `claude/slack-session-0iypwo`, issue #4 follow-up):** the 64 connector-separated candidates previously left open were each **individually adjudicated** with Ukrainian linguistic judgement: **64 confirmed P1, 0 benign, 0 needs-human** — every case a generation artifact (an English near-synonym/doubled pair collapsed to a single Ukrainian lexeme: "X або X", "X чи X", "X і X"), none a legitimate homograph/idiom/binomial/grammatical-form distinction. Corrected as a **new guarded post-migration stage** (`scripts/qa/apply_t5b_connector_fixes.py` + `t5b_connector_corrections.json`): fail-closed input blob `bb173f36` → new pinned output **`7520f722…`**; workbook untouched, base blob `4ed00c96` and earlier pins (`9282d201`, `bb173f36`) **not repinned** (D-028, now Approved). `tests/g4a_t5b_repeat_guard.py` extended to fail closed on connector-separated repeats (seeded synthetic connector defect proven caught: exit 1 → 0); `tests/g4a_t5b_supplemental_accounting.py` reports historical (314, frozen) vs supplemental P1 separately; `tests/g4a_sb0773_source_chain.py` reproduces the chain ending at `7520f722` with no earlier repin. Supplemental register now **99 corrected (35 adjacent + 64 connector) + 2 benign; 0 open, 0 needs-human**; historical `G4A_UKRAINIAN_QA_FINDINGS.csv` (142/314/246) untouched. **Validation this session: 29/29 commands PASS** (18 non-browser + 11 browser at 320/375/430/768/1024/1440) + seeded negative. Fresh T5-B review: 64 corrected items re-review clean; **fresh blind stratified sample n=48, NEW seed `913377`, POS-stratified over 1,683 clean entries → 0 new P0/P1**; seeded-defect meta-validation 6/6. **Zero unresolved supplemental P0/P1**, so the disposition is **proposed** as `G4-A PASS CANDIDATE` — proposal only; canonical gate language unchanged (`G4 technical PASS · G4-A CHANGES REQUESTED · G5 BLOCKED`) pending reviewer + Dalton sign-off. PR #7 unmerged; issue #4 not closed; G5 not started; no G4-A PASS recorded. See D-028 update and `docs/phase_4a_t5b_promotion_report.md`.
**T5-B follow-up — PR #7 blocking-review remediation (2026-09-13, branch `claude/slack-session-krpw7b`, issue #4):** an independent PR #7 review disputed the seed-913377 "0 new P0/P1" claim and named concrete defects; all are verified and corrected. (1) `connector_repeats()` in `tests/g4a_t5b_repeat_guard.py` was whitespace-only despite promising punctuation; made **punctuation-aware**, and a full-bank re-scan caught exactly the two flagged misses `SB-0364` ("міста, або міста") and `SB-0818` ("занепокоєність, або занепокоєність") — both confirmed P1, corrected. (2) `tests/g4a_sb0773_source_chain.py` made **CRLF-portable** (preserves checkout bytes byte-exact; validates the LF-normalized blob) — LF and simulated-CRLF checkouts both pass, corrupt endings fail closed; `scripts/migrate_vocabulary.py` confirmed to `write_bytes(...\n)` for both artifacts. (3) A **durable blind-sample selector** `scripts/qa/select_blind_sample.py` → `docs/G4A_T5B_BLIND_SAMPLE_20260912.json` records seed 20260912 / pool 1784 / largest-remainder POS allocation / the exact 48 IDs. (4) Reviewer blind-review defects re-adjudicated and corrected: `SB-1698` (**P0**), `SB-0005`, `SB-0744`, `SB-1023`, `SB-1517`. (5) All 64 connector-corrected entries re-adjudicated under the full rubric: 56 confirmed correct, **8 carried a further P1** now fixed (`SB-0085, SB-0448, SB-0617, SB-0684, SB-0951, SB-1015, SB-1446, SB-1657`). (6) The seed-20260912 n=48 blind sample found **0 new P0, 8 new P1**, all corrected (`SB-0175, SB-0255, SB-0349, SB-0380, SB-1029, SB-1107, SB-1336, SB-1726`). All corrections applied as a **fourth guarded post-migration stage** (`scripts/qa/apply_t5b_followup_fixes.py` + `t5b_followup_corrections.json`): input blob `7520f722` → new pinned output **`1c184e84…`**; workbook untouched, base `4ed00c96` and earlier pins not repinned (D-028). Supplemental register now **116 rows: 114 corrected (1 P0 + 113 P1) + 2 benign; 0 open**; historical `G4A_UKRAINIAN_QA_FINDINGS.csv` (142/314/246) untouched. **Validation: 21 non-browser + 10 browser (320/375/430/768/1024/1440) all PASS.** **Zero open P0/P1 across all classes**, but the canonical gate language stays `G4 technical PASS · G4-A CHANGES REQUESTED · G5 BLOCKED` — Claude proposes, reviewer + Dalton decide. PR #7 unmerged; issue #4 not closed; G5 not started; no G4-A PASS claimed. See D-028 (2026-09-13 update) and `docs/phase_4a_t5b_promotion_report.md` §22–26.
