...
**T2 progress (2026-09-09):** the first P1 batch — 104 corrections (circular-definition 26 / grammar 60 / other 18) — is **applied** to `web/vocabulary.js` via the guarded, reproducible pipeline (input blob `414fbeac`, output blob `fe46b30a`, fixed review date 2026-09-09, idempotent fail-closed); the deterministic gate and full regression packet re-run green. Unresolved P1 backlog is now **207** (311 − 104). See `docs/G4A_P1_BATCH_STATUS.md` and `scripts/qa/p1_t2_corrections.json`.
**T3 progress (2026-09-09):** the second P1 batch — **102** byte corrections (pedagogical-accuracy 49 / other 30 / russianism-calque 8 / semantic-fidelity 8 / register 7) — is **applied** to `web/vocabulary.js` via the same guarded pipeline (input blob `fe46b30a`, output blob `5cda209d`, fixed review date 2026-09-09, idempotent fail-closed); deterministic gate + full regression packet re-run green. Of the 104 T3 draft records, **SB-0425 (efficiency)** is excluded as a copy-through (its finding is resolved by sibling **SB-0424 (effectiveness → результативність)**), and **SB-0773 (minute2)** is a deferred word-field fix — its `word` should read "minute" but the rename is blocked by the g2 casefold word-uniqueness assertion, so it remains an unresolved P1 to handle with/after T4. T3 therefore resolves **103** findings (102 edited + SB-0425). Unresolved P1 backlog is now **104** (207 − 103), i.e. T4's 103 + the deferred SB-0773. See `docs/G4A_P1_BATCH_STATUS.md` and `scripts/qa/p1_t3_corrections.json`. T4 remains pending.
...
`web/vocabulary.js`; ticket **T3 (2026-09-09)** applied the second P1 batch of **102**
byte corrections (SB-0425 excluded as a copy-through resolved via SB-0424; SB-0773 deferred
as a word-field fix), resolving **103** findings. Unresolved open counts are now
**104 P1 + 246 P2** (aggregate 142 P0 / 314 P1 / 246 P2 register totals unchanged; the
register still counts all 314 as P1). The deferred SB-0773 word-field fix is included in
the remaining 104 P1.
